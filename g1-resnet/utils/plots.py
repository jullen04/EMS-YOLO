...
# def plot_images(images, targets, fname='images.jpg', names=None, max_size=1920, max_subplots=16):
...
#         # result=np.mean(images[i],axis=0)#[C,H,W]
#         result=images[i]
#         result=result.reshape(320,320,3)
#         for j in range(targets.shape[0]):
#             if (targets[j,0]==i):
#                 size=(int(targets[j,2]*320),int(targets[j,3]*320),int(targets[j,4]*320),int(targets[j,5]*320))
              
#                 pt1=(size[0]-size[2]//2,size[1]-size[3]//2)#x1,y1
#                 pt2=(size[0]+size[2]//2,size[1]+size[3]//2)#x2,y2
#                 class_id=int(targets[j,1])
...
#                 cv2.putText(result, class_name, (center[0], pt2[1] - 1), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color)

#         cv2.imwrite(str(fname)+str(i)+'.png',result)
