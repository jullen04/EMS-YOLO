...
def create_dataloader(
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        ...
        dataset = LoadImagesAndLabels(
            path, sample_size, T, image_shape, mode, batch=batch_size
        )
    ...
    )  # 定义取batch的方法，是一个迭代器， 每次生成一个key 用于读取dataset中的值
    loader = (
...
def g1_img2labelpaths(img_paths):
    # CARLA Fix: Mappar /images/img_XXXX.npy till /labels/img_XXXX.npy
    return [
        x.replace(os.sep + "images" + os.sep, os.sep + "labels" + os.sep)
        for x in img_paths
    ]
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        self, path, sample_size, T, image_shape, mode="train", prefix="", batch=16
    ):
        self.mode = mode
        ...
        self.quantized_w = (
            self.w // self.quantization_size[1]
        )  # 304 quantization_size=1
    ...
    def load_image(self, i):
        ...
        if im is None: 
            ...
            # CARLA Fix: Ta bort deras trasiga resize och im[0]-bugg!
            # Transponera direkt från [T, H, W, C] till [T, C, H, W]
            out_img = np.transpose(im, [0, 3, 1, 2])
            return out_img
    ...
    def collate_fn(batch):  # 定义数据拼接方式
        ...
        for i, l in enumerate(label):
            l[:, 0] = (
                i  # add target image index for build_targets()这个地方更换了label的第一维的值！！！
            )
...
def verify_image_label(args):
    ...
    try:
        if os.path.isfile(lb_file):
            nf = 1
            # CARLA Fix: Läs vår .npy-fil istället för .txt
            lb = np.load(lb_file)
            nl = len(lb)
            if nl:
                assert (
                    lb.shape[1] == 5
                ), f"labels require 5 columns, {lb.shape[1]} detected"
                assert (lb >= 0).all(), f"negative label values {lb[lb < 0]}"
                _, i = np.unique(lb, axis=0, return_index=True)
                if len(i) < nl:
                    lb = lb[i]
        ...
        # CARLA Fix: Ge rätt originalupplösning till YOLO!
        return im_file, lb, [480, 640], segments, nm, nf, ne, nc, msg
